...
def check_for_new_data(cursor):
    ...
    cursor.execute("SELECT MAX(id) FROM input_loadtests")
    latest_primary_key = cursor.fetchone()[0]

    if latest_primary_key != last_primary_key:
        cursor.execute(f"SELECT id, user_id, server_address FROM input_loadtests WHERE id = {latest_primary_key}")
...
def get_latest_data(cursor, id_user):
    ...
    query = f"SELECT * FROM input_loadtests WHERE user_id = {id_user}  ORDER BY id DESC LIMIT 1"
...
def pengukuran(server_address, request_per_second, connection_count, loop=5):
    ...
    for measurement in range(1, loop + 1):
        ...
        #print("hasil pengukuran :", time_taken_match)
...
def upload_pengukuran(conn, cursor, waktu_format, user_id, server_address, request_per_second, connection_count, algoritma_LB, field_chart, urutan_pengukuran, avg_time_taken_test, avg_request_loss_test, avg_request_per_second, avg_time_per_request1, avg_time_per_request2, avg_transfer_rate, avg_connection_time, test_id):

    insert_query = "INSERT INTO data_loadtests (user_id, server_address, request_count, connection_count, time_taken, request_second, time_request, transfer_rate, connection_time, request_loss, algortima_LB, field_chart, urutan_pengukuran, input_test_id, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    data = (
        user_id,
        server_address,
        request_per_second,
...
def handle_user_input(user_data):
    ...
    algoritma_LB = user_data[7]
    field_chart = user_data[8]
    urutan_pengukuran = user_data[9]
    test_id = user_data[10]
